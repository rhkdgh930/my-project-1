...
## Post Delete Image Detach 정책

- Post delete 자체는 Post 도메인의 hidden soft delete로 처리한다.
- 삭제 성공 후 발행된 `POST_DELETED` Outbox event를 통해 이미지 detach를 수행한다.
- `PostDeletedHandler`는 `imageService.syncImages(postId, POST, emptyList, userId)`를 호출한다.
- 빈 storageKeys로 sync하면 삭제된 게시글의 기존 `USED` 이미지는 `DETACHED` 처리된다.
