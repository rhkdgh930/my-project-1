...
- 파일 삭제 성공 후에만 DB 상태를 `DELETED`로 변경한다.
...
- contentType allowlist와 magic byte 검증을 함께 사용한다.
- PNG, JPEG, GIF, WebP의 최소 signature를 검증한다.
- 서비스 코드의 5MB 제한과 multipart `max-file-size`, `max-request-size` 제한을 함께 유지한다.
- 로컬 저장 경로는 upload root를 absolute normalize한다.
- `uploadRoot.resolve(storageKey).normalize()` 결과가 upload root 밖으로 나가지 않도록 `startsWith`로 검증한다.
